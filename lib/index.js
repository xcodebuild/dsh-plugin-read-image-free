 ...
 *
 * 分隔符：核心把多个 text 块拼接给模型时不插分隔符，因此连续图片的路径会
 * 合并进同一个 text 块、用单个空格分隔（避免 /a.png/b.png 黏连）；图片块
 * 前有其他文本块时，路径前补一个空格，避免与前文黏连（如 "看图/a.png"）。
 * @param ctx - 插件作用域。
...
export async function rewritePromptContent(ctx, attachments, content) {
  ...
  for (const part of content) {
    ...
    const text = path ?? '（无法保存图片）';
    if (lastWasImage) {
      // 连续多张图：合并进同一个 text 块、空格分隔，避免路径黏成 /a.png/b.png。
      out[out.length - 1].text += ` ${text}`;
    } else if (out.length > 0) {
      // 图片块前有其他文本块：前置空格，避免路径与前文黏连（如 "看图/a.png"）。
